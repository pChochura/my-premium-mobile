...
import com.pointlessapps.mypremiummobile.remote.datasource.payments.dto.PaymentResponseDto
...
internal fun List<PaymentResponseDto>.toPaymentsResponse(dateParser: SimpleDateFormat) = map {
    PaymentResponse(
        paymentDate = requireNotNull(dateParser.parse(it.paymentDate)),
        postingDate = requireNotNull(dateParser.parse(it.postingDate)),
        amount = it.amount,
        title = it.title,
    )
}
